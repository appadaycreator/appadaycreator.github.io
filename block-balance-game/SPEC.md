...
### v1.20（2026-06-20）- /improve_auto 深層品質レビュー（V2・V3 UI改善）
**改善内容**:
- **P1 (V2)**: 難易度選択ボタンの視認性大幅改善
  - パディング `4px 10px` → `7px 14px`（タップ領域拡大）
  - フォントサイズ `0.72rem` → `0.82rem`（読みやすさ向上）
  - ボーダー太さ 1.5px → 2px
  - アクティブ状態に `box-shadow` 追加（選択中の視覚的強調）
  - アクティブ状態に `✓` プレフィックス追加（選択状態を明確化）
  - 非アクティブ時は `background: #f9fafb; color: #6b7280` で明確に区別
- **P2 (V3)**: モードボタン・キャンバスヒント文の役割明確化
  - モードボタン（通常/⏱60秒）の前に「モード:」ラベルを追加
  - キャンバスのヒント文を「ブロックをドラッグして配置するとゲームスタート！」に変更
  - ヒント文の下に「↑ 難易度・モードを選んでからスタートしよう」の補足テキスト追加
  - 英語翻訳も対応更新
