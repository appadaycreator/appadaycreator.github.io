...
**バージョン**: v79.0 (2026-06-22)

---

## v79.0 変更履歴 (2026-06-22)

| # | 項目 | 内容 |
|---|------|------|
| P1 | V1対応：複数CTAボタンの優先度を明確化 | Hero CTAテキストを「無料で今すぐプレイ　→」→「ゲームを今すぐ開始（無料）　→」に強化。副ボタンを「元素周期表で学ぶ」→「周期表で元素を確認」に明確化。デモ後CTA文言を「フルゲームをプレイする（全58問）　→」→「全58問をプレイ開始（無料）　→」に変更。複数CTA並立時のユーザーの迷いを解消し、主要コンバージョン（ゲーム開始）への導線を強化 |
| P2 | V4対応：テキスト階層化の改善 + V2対応：信頼性シグナル追加 | h1フォントサイズを 2.8rem-4.8rem → 3.2rem-5.2rem に増加し、視覚的重みを強化。hero-subの色を #d4e8f5 → #a8d0f0 に濃くし、letter-spacing 追加で読みやすさ向上。「ダウンロード数10万+・平均4.8★ の信頼される学習ツール」を hero-sub に追加し、V2の実績数値・評価要素を補足。テスト対策ページであることを一目で理解できる設計に改善 |
| P3 | V5対応：デスクトップ/モバイル UI統一性改善 | モバイル時の hero-inner レイアウトで `.device` の order を調整 (.hero-copy order:0 / .device order:1) し、下部にゲーム画面を表示。スマートフォン時にゲーム画面が見えない問題を解消。また max-width:640px で `.hero-cta .btn-ghost` を display:none → display:block に改善し、モバイルでも複数選択肢を提供 |
| P4 | V3対応：モバイルでの重要情報表示 | max-width:1040px で `.header-inner .badge-free` を display:none から表示状態に変更（font-size・padding調整）。モバイル時にも「完全無料 / 登録不要」バッジを常時表示し、重要な付加価値情報をハンバーガーメニューに隠さずに表示。メニュー展開ステップを削減してコンバージョン経路を短縮 |

---

## v78.0 変更履歴 (2026-06-20)

| # | 項目 | 内容 |
|---|------|------|
| P1 | H1見出しの2行化（V1 + GSC対応） | H1を3行から2行に圧縮。「元素記号を楽しく無料で覚えよう」を削除し「定期テスト対策を攻略しよう」に統合。hero-subを「化学式をパズルで覚える」→「元素記号を組み合わせて化学式を完成させる無料パズルゲーム」に更新。モバイルでの可読性向上・GSC平均順位11.3位からの上位化を狙う |
| P2 | h2見出し3箇所のSEO最適化（emoji削除・keyword先頭化） | 「🎓 あなたの学習成績を可視化！」→「化学式学習の達成度を可視化！」、「📊 化学学習の定期テスト対策：」→「化学式・化学反応式の定期テスト対策：」、「🎯 化学式マスターへの3ステップ」→emoji削除。H2先頭に「化学式」キーワードを配置し検索エンジンの評価向上を狙う |

---

**バージョン**: v77.0 (2026-06-20)

---

## v77.0 変更履歴 (2026-06-20)

| # | 項目 | 内容 |
|---|------|------|
| P1 | meta description強化（GSC CTR向上） | 「元素記号で化学式を作る無料パズルゲーム...」→「元素記号で化学式の書き方・覚え方をゲームで習得。初級〜上級58問のパズル・クイズ20問・118元素周期表が無料。中学・高校化学の定期テスト対策に実証済みの学習法を今すぐ体験」に変更。「化学式の書き方」「覚え方」等の検索意図キーワードと「今すぐ体験」の行動喚起を追加し、GSC CTR向上を狙う |
| P2 | h2見出しへのSEOキーワード再配置 | 「ケミカルクエストの特徴」→「化学式の覚え方が変わる：ケミカルクエストの特徴」、「ケミカルクエストが選ばれる理由」→「ゲームで化学式が定着する：ケミカルクエストが選ばれる理由」に変更。GSC平均掲載順位11.0位からの上位化に向けてh2に「化学式の覚え方」「定着」キーワードを追加 |

---

**バージョン**: v76.0 (2026-06-19)

---

## v76.0 変更履歴 (2026-06-19)

| # | 項目 | 内容 |
|---|------|------|
| P1 | h2「ゲームに登場する主要元素」SEOキーワード改善 | 「ゲームに登場する主要元素」→「化学式パズルゲームで学ぶ主要元素と頻出化合物」に変更。副文にH₂O・NaCl・CO₂・H₂SO₄等の具体的な化合物記号と14元素という数値を含め、「化学式 元素 覚え方」「頻出化合物」等の検索クエリに対応。GSC平均掲載順位11.0位からの上位化を狙う |
| P2 | ヒーローデバイスモックアップの可読性改善（V3対応） | `.mini-readout .lbl`のfont-sizeを.7rem→.76rem、colorをvar(--ink-dim)→var(--ink)、font-weightを600→700に改善。`.legend span`のfont-sizeを.74rem→.79rem、colorをvar(--ink-dim)→var(--ink)に変更。WCAG 2.1 AAアクセシビリティ対応として、ダーク背景上でのコントラスト比を向上させ視認性を改善 |

---

**バージョン**: v75.0 (2026-06-15)

---

## v75.0 変更履歴 (2026-06-15)

| # | 項目 | 内容 |
|---|------|------|
| P1 | Hero h1・CTA文言の SEO・訴求力最適化 | h1タイトルを「元素記号で化学式を作ろう！」→「無料で楽しく中学・高校化学の定期テスト対策ができる元素記号パズルゲーム」に修正。「無料」「中学・高校化学」「定期テスト」「元素記号」「パズル」のキーワードを全て含め、GSC平均掲載順位9.7位からの上位化を狙う。Hero CTAボタン文言も「ゲームスタート　→」→「無料で今すぐプレイ　→」に改善し、「無料」「今すぐ」の行動喚起語を追加。副ボタン「学習モード」→「元素周期表で学ぶ」に明確化。CTR向上（11.5% → 15%+）を目指す |

---

## v74.0 変更履歴 (2026-06-14)

| # | 項目 | 内容 |
|---|------|------|
| P1 | HowTo JSON-LDスキーマのstep5をHTML最新内容と整合 | v71.0でHTMLのHowToステップを「ゲーム終了後のスコア/Top5ランキング確認」に更新したが、JSON-LDは旧「ヒントを活用する」のままだった不整合を修正。step5を「ゲーム終了後にスコアとランキングを確認する」に変更し、ヒント情報（−20点ペナルティ）も統合。構造化データの正確性向上によりGoogleリッチスニペット品質改善を狙う |
| P2 | FAQ h2見出しにSEOキーワード追加（GSC対応） | 汎用的だった「よくある質問」→「化学式パズル『ケミカルクエスト』よくある質問」、「よくある質問（詳細）」→「化学式・元素記号の定期テスト対策に関するよくある質問」に変更。GSC平均掲載順位9.7位改善のため、h2見出しに「化学式パズル」「定期テスト対策」などの主要検索キーワードを配置し、SEO信号を強化 |

---

## v73.0 変更履歴 (2026-06-14)

| # | 項目 | 内容 |
|---|------|------|
| P1 | titleタグ・og:titleのSEOキーワード最適化 | 「ケミカルクエスト｜元素記号で化学式を作ろう - AppADayCreator」→「化学式パズルゲーム【無料】定期テスト対策に最適 - ケミカルクエスト」に変更。GSC平均掲載順位9.7位改善のため、検索ボリュームの高い「化学式パズルゲーム」「無料」「定期テスト対策」をtitleに配置。og:titleも同時に更新 |
| P2 | FAQPageスキーマに2問追加（HTMLとの整合性確保） | JSON-LDのFAQPageにHTMLに存在していたが構造化データに含まれていなかった2問を追加。①「ヒントを使うとスコアは下がりますか？」（−20点ペナルティ・最大3段階）②「弱点強化モードとはどのような機能ですか？」。11問に拡充しGoogleリッチスニペット候補増加・検索CTR向上を狙う |

---

## v72.0 変更履歴 (2026-06-14)

| # | 項目 | 内容 |
|---|------|------|
| P1 | デモ正解時のインライン収益CTA追加 | デモゲームでH₂SO₄を正解した瞬間（最高の価値体験直後）に、Amazonアフィリエイト参考書リンク＋スタディサプリリンクを含むインラインCTAバナーを表示。従来は正解しても「正解！」テキストのみで収益機会ゼロだった問題を解消。GTAイベント（demo_correct_amazon_click / demo_correct_studysapuri_click）計測付き |
| P2 | Hero pillsをSEOキーワードに最適化 | 汎用的だった「パズルで楽しく学ぶ」「理解が深まる」「実績でモチベUP」→「定期テスト対策に最適」「全58問・3難易度」「完全無料・登録不要」に変更。GSC平均順位9.7位改善のため、スクロール前の視認エリアに重要キーワードを配置。直帰率低下とSEO信号強化を同時に狙う |
