...
| バージョン | 日付 | 主な変更内容 |
|-----------|------|------------|
| v11.0.17 | 2026-07-02 | /improve_phase2_only実行（Phase2直接実装版）・V1: 「85」健康スコアに「スコア（0〜100点）」説明ラベルを追加・V3: 成長タイムラインの4つの植物写真に個別の説明テキスト＆成長段階ラベル追加（「初期段階：芽の出始め」「成長加速：茎の伸長」等）・タイムラインセクションに role="region" aria-label を追加・矢印（→）に role="img" aria-label で説明文を追加・V4: 記録方式セクションの各カード（直接入力・紙のデジタル化・Excelインポート・PDF出力）に「こんな人向け」タイプラベルを追加・各メソッドの説明テキストを拡充・V5: hero ボタン「今すぐはじめる」を「今すぐアプリを使う」に明確化・ボタンに aria-label で「新規ユーザー向け」と「既存ユーザー向け」を区別・機能確認ボタンに emoji + aria-label 追加・M20: レビューセクションの星評価に role="img" aria-label を追加・レビューアバター emoji に role="img" を追加・AI診断セクションのアイコン3つに role="region" aria-label を追加・スケジュール管理写真 alt 属性を詳細化・FAQ初項目の aria-label にキーボード操作説明（Enter/Space）を追加
| v11.0.15 | 2026-06-26 | /improve_phase2_only実行・M20: アクセシビリティ対応（alt属性改善）・ヒーローアイコン（5個）alt属性を装飾的テキストから説明的テキストに更新（「登録不要・無料」「複数の植物を管理できます」等）・フィーチャーバッジ（5個）alt属性の説明化・AI診断セクションアイコン（3個）alt属性に説明文を追加・比較セクションアイコン（2個）・比較機能グリッド（4個）のalt属性を説明文に更新・合計13個の画像要素のアクセシビリティを改善
| v11.0.14 | 2026-06-25 | /improve_phase2_only実行・M20: alt="装飾アイコン"を適切なalt=""に修正（比較機能グリッド4件）・スコアカードにaria-label/role="progressbar"追加・V2: 「85」スコアに「登録植物4株の総合健康スコア」「全4株の平均値」コンテキスト追加・V5: ユーザーレビューセクション追加（利用者3名の声・月間利用者1200+・平均評価4.8など信頼性シグナル）・V1/V4: モバイルヒーロー表示順をCSS order属性で再整列（CTA→バッジの順）・V3: アイコンバッジにcursor:default追加・hero-iconセクションにaria-label追加 |
