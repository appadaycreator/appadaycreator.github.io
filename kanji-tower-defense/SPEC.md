...
### v1.9.3 (2026-06-22)
- improve (V1): 難易度ボタンをボタンデザイン化・グラデーション背景・ホバーエフェクト追加（視認性・クリック感を大幅向上、選択難易度ボタンを強調表示）
- improve (V2): シェアセクションを黄色背景で強調・見出しを「🎯 スコアをシェアして友達に自慢しよう！」に変更・CTA文言を「Xで成績をシェア」「LINEに共有」と行動喚起型に強化（リテンション・拡散効果向上）
- improve (V2): 外部リンクセクションのタイトルを「漢字学習に役立つリンク」から「📚 参考リンク」に変更・優先度を下げる（フローをシェア→参考の順序に統一）
- improve (V3): モバイル時の難易度ボタンをコンパクト表示（padding・font-sizeを調整、ファーストビューでゲーム開始までの導線が明確に）
- improve (V4): ベストスコア初期表示を「---」から「ゲームから記録」に変更（プレイ欲求喚起）
- improve (V5): ステータスバーをカラーコーディング（GOLD・HPを強調表示・フォントサイズ拡大、重要な数値が一目でわかるように）
- improve (V5): WAVE・SCORE等の補助情報を半透明にしてビジュアル階層化（視認性・スキャン性向上）
- improve: CTA文言強化「もう一度プレイ」→「🎯 スコアを更新する」（再プレイ欲求・リテンション向上）

### v1.9.2 (2026-06-20)
- fix (V3): Cookie同意バナーのlocalStorageキー不整合修正（cookie-consent-v2とlegacy cookie-consentの両方を保存・チェックするよう修正、再訪時に再表示されるバグを解消）
- improve (V3): 同意バナーボタンを緑色（#22c55e）に変更・サイズ拡大・シャドウ追加で視認性大幅向上
- improve (V1): ゲーム画面と操作パネルの境界を明示する「▼ 難易度・ゲーム操作」セパレーターを追加、ゲームエリアとコントロールパネルの区切りが明確に

### v1.9.1 (2026-06-13)
- feature: M3 データ保存・履歴機能（localStorage）- index.htmlにインラインスクリプトを追加し、ページロード時に localStorage からプレイ履歴・ベストスコア・最高WAVE・平均正答率を即時反映（main.jsロード前でも表示可能に）

### v1.8.9 (2026-06-10)
- feature: 難易度シミュレーター機能（M7）追加（「比較する」ボタンで9難易度の推定スコア・敵数・難度・勝率を並べて比較表示、難易度選択の支援）
- feature: シミュレーション結果テーブル（難易度別の推定スコア・敵数・難度ランク・推定勝率を一覧表示、ユーザーが難易度を選択しやすくなる）
- feature: スコア比較グラフ（Chart.js不使用で軽量実装、各難易度のスコア高さをビジュアル比較、ブラウザ内処理）
- feature: 推定勝率プログレスバー（難易度ごとの推定クリア確率を色分け表示（green/orange/red）、ユーザーが自分のレベルを選択しやすくなる）
...
- v1.9.0 (2026-06-13): M3 データ保存・履歴機能強化 — updateHistoryDisplay でベストスコア・最高WAVE・平均正答率を localStorage から復元表示、kjGet/kjSet ヘルパー追加
