...
- v2.0.7 (2026-06-25): M2・V1〜V5 フェーズ2追加実装
  - V4: 習い事カード価格に上限金額を追加（月N円〜M円 で最大金額の不透明性を解消）
  - V1: ステップフロー進行バーを追加（1年齢→2予算（任意）→3習い事→4費用確認）
  - V2: ヒーロー信頼性数値に出典注記追加（独自調査・n=1,200 明示）
  - V3: ヘッダーCTA「↓ 計算開始」→「↓ 計算を始める」に統一
  - V5: 計算結果後に「おすすめスクールを見る →」CTA導線を追加
  - M2: 習い事未選択時の入力ガイドメッセージを追加（⚠️カードをタップして選択）
