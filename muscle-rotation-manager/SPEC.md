...
- HowTo構造化スキーマを筋トレローテーション専用に修正 ✅
  - 「記録する情報を入力する」等の汎用テンプレートを削除
  - 「トレーニング部位を選択する」→「種目・重量・セットを記録する」→「回復状況を確認する」→「推奨部位でトレーニング」→「カレンダーで進捗管理」の5ステップに改訂
  - 大筋群72時間・小筋群48時間の回復管理など筋トレ固有の内容を記述
- r31-use-cases セクションを筋トレローテーション管理専用に刷新 ✅
  - 「血圧・血糖値・BMI管理」「生活習慣病が気になる方」等の完全無関係コンテンツを削除（AdSense品質リスク対策）
  - 「ジムを始めたばかりの初心者」「全身法から分割法へ移行したい方」「オーバートレーニングを防ぎたい方」「トレーニング記録を継続したい方」の4ユースケースに改訂
- 下部FAQを5問に拡充・品質改善 ✅
  - 「muscle-rotation-ma」バグ修正
  - 汎用的な3問を削除し、筋トレローテーション固有の5問（回復期間計算・分割法の組み方・スマートフォン利用・データ削除方法）に改訂
- ブログリンクを筋トレ関連コンテンツに変更 ✅
  - 「株式投資の始め方」「新NISA vs iDeCo」等の完全無関係リンクを削除
  - 「筋トレ分割法の基本」「オーバートレーニング症候群の予防法」「プロテイン摂取タイミング」に変更

### v1.21.0 の改善（2026-06-20）
- モバイルハンバーガーメニュー機能実装 ✅
  - ハンバーガーボタン（☰）にクリックハンドラ追加（aria-expanded管理）
  - モバイル専用ドロップダウンナビゲーション追加（全セクションリンク + 「今すぐ無料で始める」CTA）
  - メニューリンクをクリックすると自動で閉じる（UX改善）
- 診断結果後アフィリエイトCTAボタン強化 ✅
  - 平文テキストリンク → オレンジ/赤グラデーションの目立つボタンに変更
  - コピー文言を「🛒 おすすめサプリ・プロテインを今すぐ確認する →」に強化
  - ホバーエフェクト（透明度変化 + 上方向移動）追加

### v1.20.0 の改善（2026-06-13）
- M10【満足】印刷・結果エクスポート機能の実装 ✅
  - 診断結果セクション：🖨️印刷・📋コピー・📥CSV ボタン
  - シミュレーション結果セクション：🖨️印刷・📋コピー・📥CSV ボタン
  - window.print()による印刷機能（ボタン非表示）
  - 結果テキストのクリップボードコピー機能
  - CSV ダウンロード機能（評価項目・結果値をCSVで出力）
  - 印刷用CSS を追加（@media printで印刷ボタン非表示・テーブル枠線表示）

### v1.19.0 の改善（2026-06-13）

### v1.2.0 の改善（2026-06-05）
- PWA theme-color を全HTMLファイルで統一: `#6b5be2` に統一
  - 一貫性が向上: PWAインストール時の見栄え改善
  - ブランドカラー統一: manifest.jsonとの整合性確保
...
### v1.18.0 (2026-06-13) - M5・M7 機能実装（診断・シミュレーション・アフィリエイト推薦）

- **M5【収益×満足】結果に連動したアフィリエイト商品推薦** ✅ 大幅強化完了
  - ユーザーの筋トレ経験・目標・頻度に基づいた診断機能を実装
  - 診断結果に応じて最適なトレーニング機器・サプリメントを動的に推薦
  - px.a8.net A8.net アフィリエイトリンク統合（複数商品推薦）
  - 拡張された製品カテゴリ：計20種類以上のトレーニング機器・サプリメント推薦
    - 初心者向け（ダンベル・プロテイン・グローブ）
    - 中級者向け（バーベル・クレアチン・BCAA）
    - 上級者向け（パワーラック・プロテインミール・高級シューズ）
    - 筋力向上・持久力・健康維持別の各セット
  - ビジュアル改善：アイコン表示・ホバーエフェクト・左ボーダー装飾

- **M7【満足】比較・シミュレーション機能** ✅ リアルタイム対応完了
  - 週間トレーニング回数のスライダー操作（1-6回）
  - 回復期間の選択（36/48/72時間）
  - トレーニング期間の設定（4/8/12週間）
  - **リアルタイムシミュレーション計算機能** - スライダー移動で即座に結果更新
  - 総トレーニング回数・部位別平均頻度・回復効率スコア・期待効果を表示
  - 入力値変更による即時再計算対応（スライダー・セレクト変更時に自動計算）

- **SEO改善**
  - メタタイトルを「筋トレ部位別メニュー・ローテーション管理【無料ツール】」に強化
  - メタディスクリプションに主要キーワード追記（ローテーション・分割トレーニング・筋肥大）
  - 構造化データ（FAQPage・WebApplication・HowTo）を既に実装済み

- **インターフェース改善**
  - ランディングページに「診断セクション」と「シミュレーションセクション」を追加
  - 口コミセクション直後に配置（ユーザーエクスペリエンス向上）
  - Tailwind CSS でスタイリング統一
  - モバイル対応レスポンシブデザイン
