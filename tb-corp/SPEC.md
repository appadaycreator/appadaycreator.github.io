...
### v2.6 (2026-06-20)
**深層品質レビュー: SEO強化・ヒーローセクションUX改善（V1+V3）**

#### 実装内容

**P1【メタdescription拡張（SEO/GSC対策）】**
- description を75文字→120文字以上に拡張し、「DXコンサルティング・AI/LLM導入支援・初回相談無料・全国リモート対応」等の主要キーワードを追記
- OG description・Twitter descriptionも同様に更新（各SNSでの見栄え改善）
- GSC表示0件の原因の一つとして、キーワード密度不足を解消

**P2【ヒーローセクション V1+V3 視認性改善】**
- V1「料金プランを見る」ボタン: `background:rgba(255,255,255,0.9)` → `background:#ffffff` + `box-shadow:0 2px 10px rgba(124,58,237,0.18)` + `font-weight:700` でグラデーション背景から明確に浮き立つ
- V3 トラストバッジ: ドット区切りテキスト → pill型バッジ3個（border+shadow+背景付き）でモバイル可読性向上、各バッジを独立した要素として認識しやすくした
